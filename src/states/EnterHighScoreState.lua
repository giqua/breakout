...
function EnterHighScoreState:render()
    ...
    love.graphics.printf('Your score: ' .. tostring(self.score), 0, 30,V_WIDTH,'center')
    love.graphics.setFont(gFonts['large'])

    if highlightedChar == 1 then
        love.graphics.setColor(103/255, 1, 1, 1)
    end
    love.graphics.print(string.char(chars[1]), V_WIDTH / 2 -28, V_HEIGTH/2)
    love.graphics.setColor(1, 1, 1, 1)

    if highlightedChar == 2 then
        love.graphics.setColor(103/255, 1, 1, 1)
    end
    love.graphics.print(string.char(chars[2]), V_WIDTH / 2 -6, V_HEIGTH/2)
    love.graphics.setColor(1, 1, 1, 1)

    if highlightedChar == 3 then
        love.graphics.setColor(103/255, 1, 1, 1)
    end
    love.graphics.print(string.char(chars[3]), V_WIDTH / 2 + 20, V_HEIGTH/2)
    love.graphics.setColor(1, 1, 1, 1)

    love.graphics.setFont(gFonts['small'])
    love.graphics.printf('Press Enter to confirm!', 0, V_HEIGTH - 18, V_WIDTH, 'center')    
end